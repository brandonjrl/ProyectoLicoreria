...
stock int NOT NULL,
precio_compra decimal (6,3) NOT NULL,
...
--)
--go
--create proc VerNombreProd
--@idProd int
--as
--select nombreProd
--from Producto_venta
--inner join Producto on Producto_venta.idProd = Producto.idProd
--where Producto_venta.idProd=@idProd
--go
...
--values (@cantidad,@valor_total,@valor_unitario,@idProd)

--go

--create proc MostrarProductoVenta
--as
--select idPV as 'ID', Producto.nombreProd as 'Producto', cantidad as 'Cantidad', valor_unitario as 'Valor por unidad', valor_total as 'Total'
--from Producto_venta
--inner join Producto on Producto.idProd = Producto_venta.idProd
--inner join Venta on Venta.idVenta = Producto_venta.idVenta
--go

--create proc EliminarProductoVenta
--@idPV int
--as
--delete from Producto_venta where idPV=@idPV
--go
...
--@ced varchar (50)
--as
--select * from Cliente where ced = @ced
--go
-------------------------------BUSCAR cLIENTE NOMNBRE
--create proc BuscarClienteNombre
--@nombre varchar (50)
--as
--select * from Cliente where nombreCliente = @nombre
--go
--------------------------BUSCARPROVEEDOR
...
where ced = @cedula
go
--exec EstadoDeuda '1724451440'



--select*from Deuda
--SELECT Deuda.idDeuda, Deuda.prenda, Deuda.fecha, Deuda.estado,Cliente.nombreCliente
--FROM Cliente
--INNER JOIN Deuda ON Deuda.idCliente = Cliente.ced;
...
set nocount on
declare @idP int
select @idP = idProd from inserted 
insert into Historial values(getdate(), @idP, 'insertado', 'Laura' )
go*/
create procedure Pas 
@canttransfer float,
@cantEfect float,
@detalle varchar(100),
@observacion varchar(100),
@total float,
@cedcliente varchar(20),
@idvendedor  int,
@cantidad int ,
@valor_total float,
@valor_unitario float,
...
begin 
	...
	insert into Pago values(@canttransfer,@cantEfect)
	declare @idpago int
	select @idpago=Pago.idPago from Pago
	insert into venta values(@detalle,@observacion,@total,@idpago,@cedcliente,@idvendedor)
	declare @idventa int 
	select @idventa=Venta.idVenta from Venta
	insert into Producto_venta values (@cantidad,@valor_total,@valor_unitario,@idProduct,@idventa)
...
--DATOS
insert into Proveedor values ('111111111001','DistroArias','2029112')
insert into Proveedor values ('111111111001','Prodispro','200000')
insert into Proveedor values ('111111111001','CocaCola','000000')
insert into Proveedor values ('111111111001','Cerveceria Nacional','000000')
insert into Proveedor values ('111111111001','GUITIG','000000')

insert into Producto values ('PILSENER 1L',22,20.84,24,1);
insert into Producto values ('PILSENER 600CC',33,13.70,16.2,1);
insert into Producto values ('PILSENER 355CC LATA',9,4.96,6,1);
insert into Producto values ('PILSENER LIGHT 175CC',60,12.75,13,1);
insert into Producto values ('SIEMBRA 1L',44,15.44,18,1);
insert into Vendedor values ('Laura');
insert into Cliente values('1727480152', 'Genesis Cuichan', '0978610514','Quito- Valle de los chillos' )
